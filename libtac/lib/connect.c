...
int tac_connect(u_long *server, int *timeout, int servers)
{
	...
	while (tries < servers) {
		...
		if ((fd = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
			syslog(LOG_WARNING, "%s: socket creation error for %s: %m", __FUNCTION__,
		...
		if (fcntl(fd, F_SETFL, flags | O_NONBLOCK)) {
			syslog(LOG_WARNING, "%s: cannot set socket non blocking", __FUNCTION__);
			tries++;
			continue;
		}
		...
		if ((retval < 0) && (errno != EINPROGRESS)) {
			syslog(LOG_WARNING, "%s: connection to %s failed: %m", __FUNCTION__,
			                inet_ntoa(serv_addr.sin_addr));
		...
		if (!select(fd + 1, &readfds, &writefds, NULL, &tv)) {
			syslog(LOG_WARNING, "TACACS+ : Connection timeout with %s\n",
			                inet_ntoa(serv_addr.sin_addr));
			if (fcntl(fd, F_SETFL, flags))
				syslog(LOG_WARNING, "%s: cannot restore socket flags", __FUNCTION__);
			tries++;
		...
		syslog(LOG_INFO, "TACACS+: connected to %s", inet_ntoa(serv_addr.sin_addr));
